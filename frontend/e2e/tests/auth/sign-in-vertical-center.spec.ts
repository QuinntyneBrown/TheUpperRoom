...
test.describe('Sign-in card vertical centering', () => {
  ...
  test('auth-wrap uses flex centering', async ({ page }) => {
    const display = await page.locator('.auth-wrap').evaluate((el) => getComputedStyle(el).display);
    expect(display).toBe('flex');
    const alignItems = await page.locator('.auth-wrap').evaluate((el) => getComputedStyle(el).alignItems);
    expect(alignItems).toBe('center');
    const justifyContent = await page.locator('.auth-wrap').evaluate((el) => getComputedStyle(el).justifyContent);
