// BUG-040: the partner-contacts panel empty-state title is a <p>;
// design uses a heading. Promote to <h2> for correct semantics
// (mirrors BUG-035..039).
...
test.describe('Partner-contacts panel empty-state title element', () => {
  ...
  test('"No contacts associated yet." is rendered as an h2', async ({ page }) => {
    const title = page.getByTestId('partner-contacts-empty-title');
    await expect(title).toHaveJSProperty('tagName', 'H2');
    await expect(title).toHaveText('No contacts associated yet.');
  });
});
