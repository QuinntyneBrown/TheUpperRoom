...
@Component({
  ...
  styles: [`
    ...
    .global-team-detail__stat { padding: 16px 20px; border-radius: 8px; background: var(--ur-bg-surface, #1e293b); border: 1px solid var(--ur-border-subtle, #334155); min-width: 140px; }
    ...
    .global-team-detail__loading-row { height: 40px; border-radius: 6px; background: var(--ur-skeleton-bg, #f1f5f9); animation: detail-pulse 1.4s ease-in-out infinite; }
    @keyframes detail-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.45; } }
    .global-team-detail__error { display: flex; align-items: center; gap: 10px; padding: 14px 16px; border-radius: 8px; margin-top: 12px; background: var(--ur-error-bg, #fef2f2); color: var(--ur-error-fg, #dc2626); border: 1px solid var(--ur-error-border, #fecaca); font-size: 0.875rem; }
  `],
})
