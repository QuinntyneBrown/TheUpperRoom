...
- Backend `Teams/AcceptInvite.cs` — runs after `RegisterCommand` (slice 02): if a `?invite=` param is present, the registration handler joins the user to that team and assigns the roles.
