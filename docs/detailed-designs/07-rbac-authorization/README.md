...
- L2-007 AC3: Angular route without role → redirected to `/no-access`.
...
## Open Questions
- Does `TeamScopeBehavior` short-circuit before `ValidationBehavior`? Default order: Logging → Validation → TeamScope → Handler. Validation first so 400s beat 403s when both apply.
