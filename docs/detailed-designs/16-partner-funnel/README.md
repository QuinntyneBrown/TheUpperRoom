...
## Components
- Backend `Partners/ChangeStage.cs` — `ChangePartnerStageCommand : ITeamScopedRequest { PartnerId, TargetTeamId, ToStage }`. Handler updates `Partner.Stage`, inserts `PartnerStageHistory` row, then publishes via `_hub.Clients.Group($"team:{teamId}").SendAsync("partnerStageChanged", { partnerId, fromStage, toStage })`.
